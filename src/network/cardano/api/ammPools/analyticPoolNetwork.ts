import { AmmPool, AssetAmount, HexString, PoolId } from '@spectrumlabs/cardano-dex-sdk';
...
import ammPoolsDataJson from './AmmPoolData.json';
...
export class AnalyticPoolNetwork {
  ...
  private request(): Promise<AmmPoolDescriptor[]> {
    return Promise.resolve(ammPoolsDataJson.filter((apd) => apd.pool.poolType === 'cfmm' && apd.pool.version !== 'v3') as any);
    // return axios
  ...
  private createAmmPool(rawAmmPool: AmmPoolDescriptor): SdkAmmPooldescriptor {
    ...
    return {
      ammPool: new AmmPool(
        ...
        rawAmmPool.pool.version === 'v4' ? AmmPoolType.FEE_SWITCH : AmmPoolType.DEFAULT,
