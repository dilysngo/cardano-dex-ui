import { AdaAssetName, AdaPolicyId, AmmPool as CardanoBaseAmmPool, AssetAmount, AssetClass } from '@spectrumlabs/cardano-dex-sdk';
...
export class CardanoAmmPool extends AmmPool {
  ...
  get id(): string {
    return `${this.pool.id.policyId}.${RustModule.CardanoWasm.AssetName.from_hex(this.pool.id.nameHex).to_js_value()}`;
  }
  ...
  get tvl(): Currency | undefined {
    return this.metrics?.tvlAda ? new Currency(Number(this.metrics?.tvlAda).toFixed(networkAsset.decimals), networkAsset) : undefined;
  }

  get volume(): Currency | undefined {
    return this.metrics?.volumeAda ? new Currency(Number(this.metrics?.volumeAda).toFixed(networkAsset.decimals), networkAsset) : undefined;
  }
  ...
  get lp(): Currency {
    return new Currency(this.pool.lp.amount, this.toAssetInfo(this.pool.lp.asset));
  }

  get x(): Currency {
    return new Currency(this.pool.x.amount, this.toAssetInfo(this.pool.x.asset));
  }

  get y(): Currency {
    return new Currency(this.pool.y.amount, this.toAssetInfo(this.pool.y.asset));
  }
  ...
  private toAssetInfo(asset: AssetClass | undefined): AssetInfo<AssetClass> | undefined {
    if (!asset) {
      return undefined;
    }
  ...
  calculateDepositAmount(currency: Currency): Currency {
    const depositAmount = this.pool.depositAmount(new AssetAmount(this.toAssetClass(currency.asset), currency.amount));

    return new Currency(depositAmount?.amount, this.toAssetInfo(depositAmount?.asset));
  }

  calculateInputAmount(currency: Currency, slippage?: number): Currency {
    const inputAmount = this.pool.inputAmount(new AssetAmount(this.toAssetClass(currency.asset), currency.amount), slippage);

    if (!inputAmount) {
      return new Currency(0n, currency.asset.id === this.y.asset.id ? this.x.asset : this.y.asset);
    }

    return new Currency(inputAmount?.amount, this.toAssetInfo(inputAmount?.asset));
  }

  calculateOutputAmount(currency: Currency, slippage?: number): Currency {
    const outputAmount = this.pool.outputAmount(new AssetAmount(this.toAssetClass(currency.asset), currency.amount), slippage);

    return new Currency(outputAmount?.amount, this.toAssetInfo(outputAmount?.asset));
  }

  shares(input: Currency): [Currency, Currency] {
    const [assetX, assetY] = this.pool.shares(new AssetAmount(this.toAssetClass(input.asset), input.amount));

    return [new Currency(assetX.amount, this.toAssetInfo(assetX.asset)), new Currency(assetY.amount, this.toAssetInfo(assetY.asset))];
  }
}
